document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function showError(field, message) {
      ...
      const existing = field.parentElement.querySelector('.form-error-message');
    ...
    function validateField(field) {
      ...
      if (fieldType !== 'checkbox' && fieldType !== 'radio' && rules.length === 0) {
      ...
      for (let i = 0; i < rules.length; i++) {
        ...
        const ruleParam = parts[1] ? parts[1].trim() : null;

        if (!validators[ruleName]) continue;

        const value = field.value;
        const isValid = ruleParam ? validators[ruleName](value, ruleParam) : validators[ruleName](value);
    ...
    const mutationObserver = new MutationObserver(function (mutations) {
      mutations.forEach(function (mutation) {
        mutation.addedNodes.forEach(function (node) {
          ...
          if (node.classList.contains(observerTargetClass)) {
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "L.C JEWELRY משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'a430b4ba-ddd9-4290-bbb4-7745d76938f7',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'a430b4ba-ddd9-4290-bbb4-7745d76938f7';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'a430b4ba-ddd9-4290-bbb4-7745d76938f7';
